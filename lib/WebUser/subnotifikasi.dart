...
class _SubnotifikasiState extends State<Subnotifikasi> {
  ...
  void fetchDataProduk() async {
    try {
      final response = await http.get(
        Uri.parse(
            'http://192.168.1.5/ProjectWebAdminRekaChain/lib/Project/read_notifproduk.php?kodeLot=${widget.selectedProject['kodeLot']}'),
      );
  ...
  void fetchDataMaterial() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
          'http://192.168.1.5/ProjectWebAdminRekaChain/lib/Project/read_notifmaterial.php?kodeLot=$kodeLot',
        ),
      );
