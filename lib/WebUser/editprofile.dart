...
class _EditProfileState extends State<EditProfile> {
  ...
  Future _getdata() async {
    try {
      final response = await http.get(Uri.parse(
          'http://192.168.1.8/crudflutter/flutter_crud/lib/readdataprofile.php'));
  ...
  Future<void> _simpan() async {
    final response = await http.post(
      Uri.parse('http://192.168.1.8/ProjectScanner/lib/API/edit_profile.php'),
      body: {
        "nip": widget.data.nip,
  ...
  Future<void> _update() async {
    final response = await http.post(
      Uri.parse(
          'http://192.168.1.8/ProjectScanner/lib/tbl_tambahstaff/create_tambahstaff.php'),
      body: {
        "nama": namaController.text,
        "jabatan": jabatanController.text,
        "unit_kerja": unitKerjaController.text,
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          await http.post(
                              ...
                              Uri.parse(
                                  "http://192.168.1.8/ProjectWebAdminRekaChain/lib/Project/edit_profile.php"));
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) =>
